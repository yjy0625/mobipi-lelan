...
def callback_lelan(LeLaN_vel_smg: Twist):
	"""Callback function for the reached goal subscriber"""
	global v_lelan, w_lelan
	v_lelan = LeLaN_vel_smg.linear.x
	w_lelan = LeLaN_vel_smg.angular.z
...
def main():
	...
	velocity_lelan_sub = rospy.Subscriber(VELOCITY_LELAN_TOPIC, Twist, callback_lelan, queue_size=1)
	...
	while not rospy.is_shutdown():
		vel_msg = Twist()
		if reached_goal:
			vel_msg.linear.x = v_lelan
			vel_msg.angular.z = w_lelan		
